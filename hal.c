/**
 * @file	hal.c
 * @brief	サンシャインのHAL（ハードウェア抽象層）ファイル
 * @author	外川，fucchi，TKR
 * @date	2020.09.23
 * 
 * @version	v1.0		2013.10.31			外川			新規（ファイルのインクルード）
 * @version	v2.0		2020.09.23			TKR				Doxygen対応	 
 */
 ...
 * @fn		文字列出力
 * @brief	"文字列長さ指定付き"/\n"のみ "CR+LF"出力を行う。
 * @param	文字列のポインタ(char)，文字列の長さ(int)
 * @return	なし
 * @author 	外川
 * @version	v1.0	2013.11.13			外川			新規
*/
...
/**
 * @fn		1文字出力用ラッパー関数
 * @brief	printfなどの低レベル出力に使用される
 * @param	Unsigned CHAR型
 * @return	なし
 * @author 	外川
 * @version	v1.0	2013.11.13			外川			新規
*/
...
/**
 * @fn		文字列入力
 * @brief	CRコードまで/最大255文字/エコーバックあり
 * @param	CHAR型ポインタ
 * @return	なし
 * @author 	外川
 * @version	v1.0	2013.11.13			外川			新規
...
/**
 * @fn		ジャイロセンサのリファレンス値（基準の値）を設定する
 * @brief	なし
 * @param	なし
 * @return	なし
 * @author 	外川
 * @version	v1.0	2013.11.26			外川			新規
*/
 ...
 * @fn		距離センサの値を取得する
 * @brief	なし
 * @param	読み出す壁センサ(enDIST_SEN_ID型)
 * @return	壁センサ値
 * @author 	外川
 * @version	v1.0		2013.12.02			外川			新規
*/
...
/**
 * @fn		エンコーダのカウント値（パルス数）を取得する
 * @brief	中間値からの差分
 * @param	右カウント値(LONG型ポインタ)，左カウント値(LONG型ポインタ)
 * @return	なし
 * @author 	外川
 * @version	v1.0		2013.12.03			外川			新規
*/
...
/**
 * @fn		DCMの回転方向をCW（時計回り）にする
 * @brief	なし
 * @param	モータID(enDCM_ID型)
 * @return	なし
 * @author 	外川
 * @version	v1.0		2013.12.03			外川			新規
*/
...
/**
 * @fn		DCMを停止する
 * @brief	PWMのHI出力中に本関数を実行すると、ピンが100%出力状態なるため、関数内でピンをクリア（Lo）する
 * @param	モータID(enDCM_ID型)
 * @return	なし
 * @author 	外川
 * @version	v1.0		2013.12.03			外川			新規
...
/**
 * @fn		DCMのPWM-Dutyを設定する
 * @brief	割り込み外から設定すると、ダブルバッファでないと歯抜けになる場合がある
 * @brief	割り込みハンドラから実行すること。Duty0%の場合モータを停止させる（PWMにひげが出る）
 * @param	モータID(enDCM_ID型)，duty比(0~1000)
 * @return	なし
 * @author 	外川
 * @version	v1.0		2013.12.03			外川			新規
*/
